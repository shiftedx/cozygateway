...
describe("capability advertisement", () => {
  it("is a vendor-scoped id with an integer version", () => {
    ...
    // 45 adds native runtime bots. 47 adds auditable ids: recorded provenance on room and 1:1
    // transcript rows (`turnId`, `messageId`, `epoch`, `cause`, `attachTurn`, `authorBot`,
    // `inReplyToId`) plus typed room `context` on the attach turn command. Every field is
    // optional and absent on rows written before 47, so a client below 47 is unchanged.
