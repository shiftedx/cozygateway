 ...
 * hello the gateway seals the nonterminal turns the peer does not carry, through the existing
 * turn transition; a turn the peer declares active is never sealed by reconciliation. A turn no
 * peer is carrying runs on ADR 0004's provisional 120 second owner-loss lease instead of the long
 * silence ceiling. An unanswered steer on a dead turn is promoted into a new durable turn with the
 * same text and media, and an orphaned commit carrying words is projected rather than discarded.
