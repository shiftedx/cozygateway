...
export class GroupRooms {
  ...
  /** The drive currently holding a room, by room key. Present means "a round loop is live", which is
   *  the room's only piece of non-durable state. */
  readonly #drives = new Map<string, Promise<void>>();
  #closed = false;
  ...
  detail(rawName: string): BotGroupDetail {
    ...
    if (room.needsYou) {
      ...
      room.needsYou = false;
      this.#emitState(room, this.#drives.has(key) ? "running" : "settled", 0);
    }
  ...
  running(rawName: string): boolean {
    return this.#drives.has(this.#key(rawName));
  }
  ...
  async settled(rawName: string): Promise<void> {
    await this.#drives.get(this.#key(rawName))?.catch(() => {});
  }
  ...
  #view(room: BotGroupRow): BotGroup {
    ...
    const state = this.#drives.has(room.key) ? "running" : room.needsYou ? "needs_you" : "settled";
    return {
      name: room.name,
  ...
  #startDrive(key: string, epoch: number): void {
    ...
    const run: Promise<void> = (async () => {
      ...
      if (this.#closed) return;
      await this.#runRounds(key, epoch);
    })().finally(() => {
  ...
  async #runRounds(key: string, startEpoch: number): Promise<void> {
    ...
    try {
      for (; round < GROUP_MAX_ROUNDS; round += 1) {
        ...
        for (const member of responders) {
          ...
          // user message, a deleted room, or the message cap all stop the loop here.
          if (current === undefined || current.epoch !== startEpoch || this.#closed) return;
          if (posted >= GROUP_MAX_MESSAGES) return;
          ...
          if (result.outcome === "spoke") {
            ...
            posted += 1;
            spoke += 1;
            if (mentionsUser(result.text)) this.#storage.setBotGroupNeedsYou(key, true);
      ...
      const final = this.#storage.botGroup(key);
      ...
      if (final !== undefined && final.epoch === startEpoch && !this.#closed) {
  ...
  async #turn(args: {
    ...
    const { key, groupName, member, members, delta, startEpoch, storedId } = args;
    let session;
    try {
    ...
    const result = await runMemberTurn({
      ...
      live: () => {
        const room = this.#storage.botGroup(key);
        return !this.#closed && room !== undefined && room.epoch === startEpoch;
      },
      log: this.#log,
    });
