(() => {
  ...
  form.addEventListener('submit',async event=>{
    event.preventDefault();const button=form.querySelector('button');button.disabled=true;status.dataset.state='pending';form.setAttribute('aria-busy','true');status.textContent='Pairing this browser…';
    try{
      ...
      if(!response.ok){status.dataset.state='error';status.textContent='The code could not be used. Create a new observer code in CozyChat and try again.';return;}
      const data=await response.json();if(typeof data.deviceToken!=='string')throw new Error('pair');
      await open(data.deviceToken);
    }catch{status.dataset.state='error';status.textContent='Pairing could not finish. Check the connection and allow this site to store its paired credential.';}finally{button.disabled=false;form.removeAttribute('aria-busy');}
  });
