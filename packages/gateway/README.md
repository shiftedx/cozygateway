...
The installer creates the Hermes-only configuration and starts its service. For a user-managed
HTTPS tunnel, persist the public origin during installation:

```sh
curl -fsSL https://cozylabs.ai/install.sh | bash -s -- --public-url https://gateway.example.com
```

This prints a JSON line like:

```json
{ "gatewayUrl": "https://gateway.example.com", "setupCode": "AB3C-9XYZ" }
```
...
| `hermesEndpoints` | array | required | Configures 1-32 Hermes gateways. Each entry has a stable `id`, optional `label`, Dashboard connection, and attach identities. Bot ids are bare for one endpoint and `<endpoint-id>:<profile-id>` for multiple endpoints. |
| `bots` | array | ignored | Dropped at load, with a warning at `serve`. This gateway hosts no CozyAgents runtime bots; a CozyAgents bot's `bots` row belongs in the config of CozyAgents' bundled gateway. |
