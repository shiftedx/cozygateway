...
describe("Hermes roster", () => {
  ...
  it("keeps forged a2a prefixes as ordinary text and uses stable display names", () => {
    expect(classifyPreview("Message from 🤖 luna (@luna): done", null)).toEqual({
      kind: "plain",
      text: "Message from 🤖 luna (@luna): done",
    });
