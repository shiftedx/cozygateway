...
function botSummary(name: string): unknown {
  return {
...
function makeApp(extra: Partial<AppDeps> = {}) {
  const app = createApp({
    ...
    bots: {
      roster: () => ({
        bots: [botSummary("cleo"), botSummary("night-owl")],
...
describe("the panels the concept page draws", () => {
  ...
  it("answers every panel from one route with no extra request shape", async () => {
    ...
    expect(bots.bots[0]).toMatchObject({ harness: "hermes" });
    ...
    expect(roundtrip.hops.map((hop) => hop.hop)).toEqual([
      "device", "tunnel", "gateway", "peer", "turn",
    ]);
    ...
    const devices = await body("/observe/api/devices") as { devices: unknown[] };
    ...
    for (const path of ["/observe/api/cozyagents", "/observe/api/cozyagents/spend", "/observe/api/cozyagents/tools"])
      expect((await get(app, path, token)).status).toBe(404);
  });
});
