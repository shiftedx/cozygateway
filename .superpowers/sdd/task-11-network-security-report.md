...
- Tailscale Serve creation and removal no longer issue port-scoped CLI mutations. The bounded named-pipe LocalAPI client reads the full ServeConfig and ETag, verifies either an empty creation slot or the exact owned non-Funnel TLS-terminated TCP handler, clones and changes only that entry, then POSTs with `If-Match`. HTTP 412 and exact-state conflicts retain ownership as `mapping_changed`; there is no CLI mutation fallback. Real HTTP socket races prove concurrent user creation/replacement survives byte-for-byte.
...
- Latest final-blocker focused suite: `tailscale-cli`, `tailscale-mode`, `windows-helper-client`, and `windows-onboarding`; 105 tests passed.
- Gateway typecheck: `pnpm typecheck`
- Gateway build: `pnpm build`
- Full Gateway suite: `pnpm test` — 100 files passed, 1 skipped; 1,215 tests passed, 2 skipped.
