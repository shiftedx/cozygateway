...
At bots capability revision 60, each `GET /runners` row adds required `updateSupported: boolean`. It is true only when that runner advertised `update_runner` in its authenticated `/runner/v1` hello; legacy runners report false. This is a projection of an explicit runner capability, not a second protocol or a promise inferred from `agentVersion`.

`POST /runners/:id/update` requires paired-device auth and exact body `{ requestId }`. It returns `RunnerMaintenanceAccepted` with 202 after the SQLite row exists, whether the named runner is online or offline; offline acceptance is a server/API property, not authorization for Chat to offer or automatically submit a new update while the row says `updateSupported: false`. `GET /runners/:id/maintenance/:operationId` requires paired-device auth and returns the exact `RunnerMaintenanceOperation` with 200. A missing runner or an operation not owned by that route's runner id returns the existing redacted 404 shape; neither response reveals whether another runner owns the operation.
...
The existing `/runner/v1` hello adds exact `capabilities: ("update_runner")[]`; `hello_ack` echoes exact `capabilities: ("update_runner")[]` as the intersection understood by both peers. Gateway accepts the durable POST independently of connection state, but sends the command only after that negotiation; it does not introduce another socket, endpoint, version, or generic job protocol.

Only terminal receipts may carry one of these stable codes: `invalid_request`, `update_in_progress`, `manifest_unreachable`, `manifest_invalid`, `asset_unreachable`, `asset_size_mismatch`, `asset_checksum_mismatch`, `promotion_failed`, `service_failed`, `readiness_timeout`, or `rollback_failed`. `invalid_request` is the bounded answer to a malformed update command and `update_in_progress` is the bounded answer when another operation is already nonterminal. `message` is optional, bounded to 240 characters, and must pass the same secret/path/command-line redaction rule as local maintenance. The command contains no version, URL, executable, credential, or free-form command; the runner's own verified updater chooses and validates its release. Gateway's stored `requestedAt` is authoritative: a receipt timestamp neither replaces it nor has to equal it, and receipt acceptance must not use `requestedAt` as an equality fence.
...
- [ ] Raise `com.cozylabs.bots` to revision 60 and add required `updateSupported` to `RunnerSchema`. Add `RunnerUpdateRequestSchema`, `RunnerMaintenanceAcceptedSchema`, and `RunnerMaintenanceOperationSchema` beside the current runner schemas in `packages/contract/src/ext-bots.ts`. Reject unknown fields, operation ids outside exact `runner_maintenance_[0-9a-f]{32}`, unbounded ids/messages, invalid steps/statuses, and any token, URL, log, argv, or environment-shaped additions. Add contract tests named `accepts the runner update request and durable operation`, `projects negotiated runner update support`, `rejects runner maintenance secrets and unknown fields`, and `accepts only the locked runner maintenance terminal codes`.
- [ ] Add `RunnerMaintenanceReceiptSchema` to `RunnerClientFrameSchema` and `runner_maintenance_receipt` to `RUNNER_CLIENT_FRAME_KINDS`. Add the exact `update_runner` member to `RunnerServerFrame`; add `capabilities: ("update_runner")[]` to both hello and hello-ack schemas and dispatch only when the intersection contains it. Document both frames, negotiation, the `runner_maintenance_<32 lowercase hex>` operation-id format, and their retry rules in `contract/runner-v1.md`; keep protocol version 1 and add no second protocol.
...
- [ ] Define monotonic receipt ordering as status `pending < running < terminal` and step `download < promote < restart < readiness`. Ignore a receipt whose status/step regresses, whose terminal code is not allowlisted, or whose operation belongs to a different authenticated runner. Never compare or copy receipt `requestedAt`: keep the Gateway row's acceptance timestamp authoritative. A second byte-equivalent terminal receipt is idempotent. Once terminal, no different receipt can change the row.
...
- [ ] On the same runner reconnect, call `resetUnfinishedRunnerMaintenanceSends(connection.key)` and resend the same `operationId` for every nonterminal row, including one that already produced a running receipt; continue until the row is terminal. The Agents side treats that id as idempotent and resumes/returns its durable receipt. A runner that is offline at POST time leaves the operation `pending/download`, `sent_at = NULL`; POST still returns 202 and the first authenticated hello that negotiates `update_runner` dispatches it. A Gateway restart obtains the same behavior from SQLite without recreating the operation.
