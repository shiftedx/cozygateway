...
A subsequent full native `npm run test:windows` on integration commit `8dcd6a3c9d87d8504f7751f8475d7cbc2787504e` exited 1. Its installer suite failed at the checksum-verified private Node staging case: the expected `node/node.exe` was absent when invoked. This later run does not replace the successful original pinned-source lane above, and does not establish full native acceptance for the integrated candidate. At that point the cause was unqualified; the failed-run log and exit-code sidecar are preserved under `<logs>/integration/native-windows-long-path.*`.

Follow-up isolation identified the fixture path depth as the cause of that failure. The deeper integration temp prefix produced a 267-character staged `node.exe` path; the original shorter `<logs>/agents/temp` prefix produced a 255-character path and the same private Node case succeeded. ZIP extraction was not the failing operation. Direct native process probes failed with Win32 error 2 for the long ordinary path and error 206 for its extended-path form. This evidence isolates a Windows executable-launch path limit and does not implicate the CLI entrypoint regression. Diagnostic JSON remains private under `<logs>/integration/` (`private-node-long.json`, `private-node-short.json`, `node-path-probe.json`, and `node-path-direct-process.json`). Deeper custom installation or staging paths remain unqualified; this does not support a blanket Windows acceptance claim.
The full native `npm run test:windows` rerun at the unchanged integration commit `8dcd6a3c9d87d8504f7751f8475d7cbc2787504e` passed with exit 0 at `2026-09-07T22:52:08Z` using the original shorter temp prefix. It emitted 130 `ok` lines: 1 entrypoint regression, 1 bundle check, 109 installer checks, and 19 lifecycle assertions. Startup fallback was still not exercised; session-transition and paired Gateway acceptance remain manual and unqualified. No source changes were needed. Evidence is retained as `<logs>/integration/native-windows-short.log` and `.json`.

Post-rerun inspection found no fixture tasks or processes; only Node's compile cache remained in the shorter temp root. Seven diagnostic roots were removed. Fixtures from the earlier failed long-path suite remain intentionally preserved under `<logs>/integration/native-temp`; the successful rerun cleanup is not a claim that those older fixtures were removed. Cleanup evidence is retained as `<logs>/integration/native-short-cleanup.json` and `diagnostic-cleanup.json`.
