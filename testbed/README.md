...
```sh
./up.sh      # gateway container, dashboard, both Hermes profiles, the runner, THEN /ready
./status.sh
```
...
itself coming up, not on `/ready`), then the dashboard, then both profile gateways, then
the runner, and only after all of that does it gate on `/ready` with the same 60-second
budget it always had. Gating on `/ready` any earlier always fails on a genuinely cold bed
(`curl: (22) ... 503` then `gateway never became ready`) even though the bed comes up fine
moments later once the rest of the pieces exist. See
...
**Do not build this way if the run needs a pairing to survive a relaunch.** An
ad-hoc, unsigned build carries no entitlements at all, so `App/CozyChat.entitlements`'
`keychain-access-groups` entitlement is gone, `SecItemAdd` fails silently, and the app shows
"no gateway yet" after a perfectly successful pairing the moment it is relaunched (TB2
defect 3). Use `./build-signed.sh` instead for that case:

```sh
COZYCHAT_SIM_UDID=<udid> COZYCHAT_DERIVED_DATA=<scratch>/tb1/DerivedData-signed \
  ./build-signed.sh
```

It wraps the exact invocation TB2 proved keeps pairing alive across a relaunch
(`CODE_SIGNING_ALLOWED=YES CODE_SIGN_IDENTITY=- CODE_SIGN_STYLE=Manual DEVELOPMENT_TEAM=
PROVISIONING_PROFILE_SPECIFIER=`), so nobody has to re-derive those four flags from a report
again.
