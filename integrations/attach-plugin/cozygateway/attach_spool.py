...
class AttachSpool:
    ...
    def enqueue_event(self, event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        with self._db:
            ...
            self._db.execute(
                "INSERT INTO event_outbox (sequence, event_id, frame_json, byte_count, acked) VALUES (?, ?, ?, ?, 0)",
                (sequence, event_id, encoded, len(encoded.encode("utf-8"))),
            )
    ...
    def ack_event(self, sequence: int, event_id: str) -> bool:
        with self._db:
            changed = self._db.execute(
                "UPDATE event_outbox SET acked = 1 WHERE sequence = ? AND event_id = ?",
                (sequence, event_id),
            ).rowcount
            if not changed:
                return False
            ...
            self._db.execute("UPDATE state SET event_cursor = ? WHERE id = 1", (cursor,))
        return True
