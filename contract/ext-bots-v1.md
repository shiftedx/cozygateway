...
| 73 | The one hop the gateway cannot measure is reported by the phone that can: `POST /bots/:name/chat/messages/displayed` gains two OPTIONAL fields, `feltLatencyMs`, an integer 0 to 600000 that is the time from the send being tapped to the first delta being rendered ON THE PHONE'S OWN CLOCK, and `networkPath`, one of `wifi`, `cellular`, `vpn_on`, `vpn_off`, read by the app from the system path monitor and the presence of a tunnel interface. THE GATEWAY CANNOT MEASURE EITHER AND NEVER WILL: it sees an admission and a frame leaving, not a thumb and a pixel, and it cannot see a VPN at all. The gateway stores both on the receipt row it already writes, nullable, and records at most ONE perceived-latency sample per request whatever the batch size, so a coalesced scroll burst can never report the same wait sixty four times; a client that has a perceived latency to declare sends the message it belongs to on its own rather than folded into a burst. NEITHER FIGURE IS EVER ADDED TO OR SUBTRACTED FROM A GATEWAY-MEASURED ONE. Two clocks that were never synchronised cannot be differenced, so the perceived figure is shown beside the measured hops and never inside them, and the cost of a VPN is stated as the difference of two medians for the SAME device with the VPN on and off, each with its own sample count, which is a measured difference rather than an estimate. A `networkPath` reported without a `feltLatencyMs` is accepted and stored, because a path with no timing still says which paths a device uses. A value outside the closed set or a negative latency is `400 invalid_request` naming the field rather than a silently dropped one, so a client learns its report was not recorded. What is NOT added: no new route, no new frame, no field on any attach frame, and nothing a peer of any backend can read or write. The measurements belong to the person's own device and this gateway, and no bot, runtime or model is told what anybody waited. Additive: both fields are optional, a client below 73 sends neither and is byte identical to its pre-73 self, a gateway below 73 ignores them as it ignores any unknown key on this body, and every peer of every backend is untouched. |
