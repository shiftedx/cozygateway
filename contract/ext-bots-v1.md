...
provenance exists. `sha256` and `mark` are therefore optional on a record: they are present exactly
when `origin` is `declared`.
...
a `derived` one omits `sha256` and `mark`.
