...
| 52 | Paired runners: `POST /pair {setupCode, deviceName, kind: "runner"}` mints a per-runner token instead of a device token, `GET /runners`, `PATCH /runners/:id {default}` and `DELETE /runners/:id` manage the roster, and `/runner/v1` accepts any active per-runner token so two computers hold two sockets at once. The runner `hello` gains optional `name`, `platform` and `agentVersion`. A gateway with no Hermes endpoint is a supported configuration; its roster answers from runtime bots and `/ready` reports the bridge as `absent`. |
...
The runner token authenticates `/runner/v1` and nothing else. It cannot read chats, list devices or
create bots, and `DELETE /runners/:id` revokes it.
