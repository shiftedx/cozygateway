...
| 69 | Stale native turn reconciliation, so a reply is never lost to a turn nobody owns. THE GATEWAY SIDE IS THE FLOOR: it protects users of any peer, including one that never sends a new field. Attach-v1 `hello` gains optional `activeTurns` (turn ids only, at most 256) and `failed` gains the closed optional `reason: "unknown_turn"`; see `contract/attach-v1.md`. On hello the gateway reconciles that profile's nonterminal native turns: a turn the peer DECLARED active is never sealed by reconciliation and keeps the long silence ceiling; a turn the peer did not name, when it declared at all, is sealed IMMEDIATELY for owner loss rather than after twenty minutes of silence, and the app learns through the existing `bot_chat_state` turn transition (`phase: "failed"`, `status: "failed"`), with no new frame, field or status value. A peer that declared nothing cannot be read either way, so its turns go on ADR 0004's provisional 120 second owner-loss lease, the same bound a Task gets: one frame from the peer keeps the turn, no frame at all ends it. The lease is also the whole silent-reap window whenever the peer is DISCONNECTED, and it never lengthens a window an operator already shortened. A STEER left unanswered on a turn that turns out to be terminal, reaped, or answered `unknown_turn` is PROMOTED into a new durable turn carrying the same text and media, and the person's own message row moves onto it, so the app sees an ordinary new turn and the reply names the question it answers; a promotion that cannot be dispatched records a visible marked failed-delivery row preserving the text instead. An ORPHANED COMMIT carrying user-facing text or media, arriving on a turn id this gateway never issued, is projected as an ordinary reply bound to no turn rather than acknowledged and discarded; an orphaned frame carrying nothing a person can read is still declined. The lease is provisional and re-evaluated with measured findings. Additive: a peer that sends neither field, and every client, are byte identical to their pre-69 selves, and no route, frame, field or status value is added. |
