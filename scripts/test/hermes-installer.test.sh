...
tmp="$(cd -P "$tmp" && pwd)"
for _ in {1..32}; do printf 'n\n'; done > "$tmp/lan-no"
for _ in {1..32}; do printf 'n\n'; done > "$tmp/pair-no"
# Every fixture gets explicit negative answers unless it overrides one below. This keeps the
# production installer prompt unchanged while preventing a missing fixture input from reading tty.
export COZYGATEWAY_TEST_LAN_PROMPT_INPUT="$tmp/lan-no"
export COZYGATEWAY_TEST_PAIR_PROMPT_INPUT="$tmp/pair-no"
stop_test_pid() {
...
for pass in first repair; do
  HOME="$escaped_home" PATH="$tmp/service-bin:$tmp/bin:$PATH" COZYGATEWAY_TEST_LAN_PROMPT_INPUT="$tmp/lan-no" COZYGATEWAY_TEST_HERMES_ROOT="$tmp/hermes" COZYGATEWAY_TEST_COMMAND_LOG="$tmp/escaped-launchd-$pass-commands" COZYGATEWAY_HERMES_BIN="$tmp/darwin-home/.local/bin/hermes" COZYGATEWAY_NODE="$fake_node" COZYGATEWAY_SERVICE_PLATFORM=Darwin bash "$repo_root/scripts/agent-install.sh" --profiles default --bundle "$escaped_bundle" --plugin-archive "$tmp/plugin.tar.gz" --gateway-dir "$escaped_gateway" >/dev/null
done
...
# Streaming repair on a host whose `config set` returns 0 without writing (a
# package-managed Hermes): the installer reads the file back, finds the keys
# still absent, and must NOT restart the profile. Trusting the exit code here
# would restart a profile on every single rerun for a change that never landed.
# The lifecycle assertion below is the one that catches it.
expect_contains "$configured_rerun_output" 'streaming settings for profile ops are still absent, so it was not restarted'
...
# A `config set` that FAILS is one profile without draft frames, never a failed
# install: the run carries on and finishes.
if ! write_failure_output="$(HOME="$tmp/darwin-home" PATH="$tmp/service-bin:$tmp/bin:$PATH" COZYGATEWAY_TEST_HERMES_ROOT="$tmp/hermes" COZYGATEWAY_TEST_COMMAND_LOG="$tmp/write-failure-commands" COZYGATEWAY_TEST_CONFIG_SET_FAILS=1 COZYGATEWAY_TEST_MODEL_DECLINE=1 COZYGATEWAY_HERMES_BIN="$tmp/darwin-home/.local/bin/hermes" COZYGATEWAY_SERVICE_PLATFORM=Darwin bash "$repo_root/scripts/agent-install.sh" --bundle "$tmp/gateway.mjs" --plugin-archive "$tmp/plugin.tar.gz" --gateway-dir "$tmp/gateway-live" 2>&1)"; then
  printf 'a failed streaming config set must not fail the install:\n%s\n' "$write_failure_output" >&2
  exit 1
fi
expect_contains "$write_failure_output" 'hermes could not set display.streaming, so streaming settings for profile ops were left alone'
expect_contains "$write_failure_output" 'CozyGateway listens on'
if grep -Eq ':gateway:(restart|start|install)$' "$tmp/write-failure-commands"; then
  printf 'a failed streaming config set restarted a Hermes profile:\n' >&2
...
# A bot created later from the phone: Hermes `profiles.create` makes the profile (with a copy of the
# launch profile's .env) and the gateway's provisioner reruns this installer. The running host must
# pick the new profile up through Hermes' own control verbs, with no stop and no restart that would
# bounce every other bot: plugins reloaded for its home, a change to its .env after that, a rescan.
...
test "$(sed -n 's/^COZYGATEWAY_SPOOL_PATH=//p' "$mux_hermes/profiles/phone/.env")" = "$mux_hermes/profiles/phone/plugin-data/cozygateway/attach-v1.sqlite"
expect_contains "$mux_phone_output" 'the host Hermes gateway picked up profiles phone without a restart'

# If the host does not answer its control verbs, the fallback is ONE host restart.
