...
$profileGrammarCases = @(
    ...
    @{ Name = 'isolated default profile'; Arguments = '-p default dashboard --isolated --port 9119'; Expected = 'Owned' },
    @{ Name = 'supervisor private fallback argv'; Arguments = 'dashboard -p default --host 127.0.0.1 --port 9119 --no-open --skip-build --isolated'; Expected = 'Owned' },
    @{ Name = 'isolated as a top-level option before the subcommand'; Arguments = '--isolated -p default dashboard --port 9119'; Expected = 'Foreign' },
