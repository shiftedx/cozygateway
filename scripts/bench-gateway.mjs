...
const WORKER = "bench";
const HISTORY_ROWS = parseCount(process.env.HISTORY, 128_000, "HISTORY");
const TOOL_COUNT = 40;
const DRAFT_COUNT = parseCount(process.env.DRAFTS, 920, "DRAFTS");
const DRAFT_INTERVAL_MS = parseCount(process.env.DRAFT_INTERVAL_MS, 0, "DRAFT_INTERVAL_MS");
...
async function run() {
  ...
  try {
    ...
    for (let draft = 0; draft < DRAFT_COUNT; draft += 1) {
      if (draft > 0 && DRAFT_INTERVAL_MS > 0)
        await delay(DRAFT_INTERVAL_MS);
    ...
    await waitFor(() => {
      ...
      return worker.eventAcks === EVENT_COUNT;
    }, TIMEOUT_MS, "durable event ACKs");
    const ackMs = performance.now() - burstStartedAt;
    await waitFor(
      () => {
        if (worker.error !== undefined) throw worker.error;
        return appSawTerminalState && appSawAnswer
    ...
    const eventAckLatency = summarizeLatencies(worker.eventAckLatencies);
    const draftAckLatency = summarizeLatencies(worker.draftAckLatencies);
    ...
    return {
      ...
      pacing: { draftIntervalMs: DRAFT_INTERVAL_MS },
      metrics: {
        ...
        rssBytes: { before: rssStart, after: rssEnd, peak: peakRss, delta: rssEnd - rssStart },
...
function parseCount(value, fallback, name) {
  ...
  if (!Number.isSafeInteger(count) || count < 0) throw new Error(`${name} must be a finite non-negative integer`);
  return count;
}
...
async function connectAttach(url, token, agentId, capabilities, eventSequence, Socket, track) {
  ...
  record.socket.on("message", (data) => {
    ...
    if (frame?.kind === "ack" && frame.channel === "event") {
      try {
        const pending = state.pendingEvents.get(frame.sequence);
        if (pending === undefined) throw new Error(`unexpected or duplicate event ACK for sequence ${frame.sequence}`);
        if (pending.eventId !== frame.id)
          throw new Error(`event ACK mismatch for sequence ${frame.sequence}: expected ${pending.eventId}, got ${frame.id}`);
        state.pendingEvents.delete(frame.sequence);
        state.eventAcks += 1;
        const latency = performance.now() - pending.sentAt;
        state.eventAckLatencies.push(latency);
        if (pending.kind === "draft") state.draftAckLatencies.push(latency);
      } catch (error) {
...
async function sendEvent(peer, sequence, eventId, event) {
  await waitFor(() => {
    if (peer.error !== undefined) throw peer.error;
    return peer.pendingEvents.size < peer.eventWindow;
  }, TIMEOUT_MS, "event window");
  ...
  peer.pendingEvents.set(sequence, { eventId, kind: event.kind, sentAt: performance.now() });
  try {
    peer.socket.send(data);
  } catch (error) {
    peer.pendingEvents.delete(sequence);
    throw error;
  }
  peer.sentEvents += 1;
}
