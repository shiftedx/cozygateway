...
function Acquire-BootstrapLock {
    ...
    Assert-BootstrapPath $script:BootstrapLockPath
    Assert-BootstrapPath (Join-Path $script:BootstrapLockPath 'pid')
    try {
        New-Item -ItemType Directory -Path $script:BootstrapLockPath -ErrorAction Stop | Out-Null
    } catch {
        $pidPath = Join-Path $script:BootstrapLockPath 'pid'
        $owner = if (Test-Path -LiteralPath $pidPath -PathType Leaf) { (Get-Content -LiteralPath $pidPath -Raw).Trim() } else { '' }
        $live = $false
        $ownerId = 0
        if ([int]::TryParse($owner, [ref]$ownerId)) { $live = $null -ne (Get-Process -Id $ownerId -ErrorAction SilentlyContinue) }
        if ($live) { Fail 'another CozyGateway bootstrap is running; wait for it to finish and rerun' }
        if (-not (Test-Path -LiteralPath $script:BootstrapLockPath -PathType Container) -or @((Get-ChildItem -LiteralPath $script:BootstrapLockPath -Force)).Count -gt 1) {
            Fail 'another CozyGateway bootstrap is running; wait for it to finish and rerun'
        }
        Remove-Item -LiteralPath $pidPath -Force -ErrorAction SilentlyContinue
        Remove-Item -LiteralPath $script:BootstrapLockPath -Force -ErrorAction Stop
        New-Item -ItemType Directory -Path $script:BootstrapLockPath -ErrorAction Stop | Out-Null
    }
    Set-Content -LiteralPath (Join-Path $script:BootstrapLockPath 'pid') -Value $PID -NoNewline -Encoding ascii
}

function Release-BootstrapLock {
    if ([string]::IsNullOrWhiteSpace($script:BootstrapLockPath)) { return }
    Remove-Item -LiteralPath (Join-Path $script:BootstrapLockPath 'pid') -Force -ErrorAction SilentlyContinue
    Remove-Item -LiteralPath $script:BootstrapLockPath -Force -ErrorAction SilentlyContinue
}
...
function Test-OwnedGatewayTask {
    param([string] $InstallRoot, [string] $TaskXml)
    ...
    if (-not [string]::Equals($exec.Command, $node, [StringComparison]::OrdinalIgnoreCase)) { return $false }
    # Task XML generated by the installer quotes every argument. Parse that exact
    # grammar rather than accepting an arbitrary command line under this task name.
    ...
    if ($values[0] -ne $supervisor) { return $false }
    $required = @('--platform', 'Windows', '--gateway-env', (Join-Path $InstallRoot 'local\gateway.env'), '--bundle', $bundle, '--config', (Join-Path $InstallRoot 'local\cozygateway.config.json'), '--maintenance-socket', '\\.\pipe\cozygateway-maintenance', '--maintenance-worker', (Join-Path $InstallRoot 'local\maintenance-worker.cjs'), '--database', (Join-Path $InstallRoot 'local\cozygateway.sqlite'))
    foreach ($value in $required) { if ($values -cnotcontains $value) { return $false } }
    return $true
}
function Restart-OwnedGatewayService {
    ...
    if ($LASTEXITCODE -eq 0 -and -not [string]::IsNullOrWhiteSpace($taskXml)) {
        if (-not (Test-OwnedGatewayTask $InstallRoot $taskXml)) { Fail 'existing Gateway Scheduled Task is not owned by this installer' }
        & schtasks.exe /Run /TN CozyGateway | Out-Null
        if ($LASTEXITCODE -ne 0) { Fail 'owned Gateway Scheduled Task did not start' }
        return
    }
    # A missing registration is valid for a fresh snapshot. Do not create one
    # here: recovery is deliberately limited to restarting an existing service.
}
