...
- The per-server MCP repair policy is declared before it is emitted (`com.cozylabs.bots` capability
  ...
  relays and never stores, writes, or executes; the profile patch has no shape for it. Absent means
  the peer reported no policy for that server, never `approve_once`. An unknown value follows the
  lane's existing convention: the `config_result` frame is refused and the read is unavailable.
